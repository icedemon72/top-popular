...
@php
    ...
    $disabledClass = 'opacity-40 select-none cursor-not-allowed';
    if(!$disabled) {
        $disabledClass = 'focus:border-blue-400 cursor-pointer';
    }
@endphp
<style>
...
<div class="relative bw-select bw-select-{{$input_name}} @if($add_clearing) mb-3 @endif"
    ...
    <div {{ $attributes->merge([
        ...
        dark:text-slate-300 dark:border-slate-700 dark:bg-slate-800 py-3.5 pl-4 pr-2 clickable ".$disabledClass
    ]) }}
         tabindex="0">
    ...
    <div class="w-full absolute z-30 rounded-br-lg rounded-bl-lg bg-white shadow-sm shadow-slate-400 border-2 
        ...
        hidden bw-select-items-container overflow-auto max-h-64 animate__animated animate__fadeIn animate__faster">
