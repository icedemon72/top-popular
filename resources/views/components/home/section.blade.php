    ...
    <div class="min-w-full flex flex-nowrap items-center pb-4 ">
        @foreach($categories as $category)
            <div class="inline-block px-3">
                <a href="{{ route('post.index', $category->id) }}" class="w-40 h-40 flex flex-col items-center justify-center rounded-lg text-main shadow-md bg-card hover:shadow-xl transition-shadow duration-300 ease-in-out cursor-pointer border border-main">
