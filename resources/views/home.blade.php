	...
	<div class="w-full flex flex-col items-center justify-center mt-2">
		<div class="w-full md:w-4/5 lg:w-3/5 flex flex-col p-4 gap-3">
			@if(count($favCategories) !== 0 && (!request()->has('page') || request()->page == 1))
				<div class="w-full flex items-center gap-2 font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight bg-white dark:bg-gray-800 p-4 rounded-lg border border-main">
					<x-lucide-star class="fill-yellow-500" />
					{{ __('Your absolute favourites') }}
				</div>
				<x-home.section :categories="$favCategories" />

				<div class="w-full flex items-center gap-2 font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight bg-white dark:bg-gray-800 p-4 mt-2 rounded-lg border border-main">
					<x-lucide-sprout />
